! DCD3 is subroutine to calculate  deformation in an elastic half space due to a
! rectangular shear or tensile dislocation.
! This has been take from the opean source package at https://www.bosai.go.jp/e/dc3d.html
! It has been produced by Y.OKADA
!
!  It has been slightly modified from original version with the conversion of all calculations
!  to double precision and the removal of common blocks
!
! Reference
...
module okada
  ...
  real*8,parameter :: F0 = 0.D0
  real*8,parameter :: F1 = 1.D0
  real*8,parameter :: F2 = 2.D0
  real*8,parameter :: F3 = 3.D0
...
real*8,intent(in) :: ALPHA,X,Y,Z,DEPTH,DIP          &
                       ,AL1,AL2,AW1,AW2
real*8,intent(out) :: UX1,UX2,UX3,UY1,UY2,UY3,      &
                        UZ1,UZ2,UZ3
real*8,dimension(3),intent(out) :: UXX,UYX,UZX,UXY, &
				UYY,UZY,UXZ,UYZ,UZZ
...
!C-----
      ...
      DO 333 J=1,2
        ...
        CALL DCCON2(XI,ET,Q,SD,CD,c2)
        CALL UA(XI,ET,Q,DUA,c0,c2)
        CALL UB(XI,ET,Q,DUB,c0,c2)
        CALL UC(XI,ET,Q,ZZ,DUC,c0,c2)
...
PURE SUBROUTINE  UA(XI,ET,Q,U,c0,c2)
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8,intent(in) :: XI,ET,Q
      real*8,intent(out):: U(12,3)
      real*8,intent(in) :: c0(12),c2(24)
      !DIMENSION U(12,3)
!C
!C********************************************************************
...
!C======================================
      ...
      U(7,1)= (ALP1*XY*SD                         &
               	+ALP2*XI*FY+D/F2*X11)/PI2
      U(8,1)=           (ALP2*EY)/PI2
      U(9,1)= (ALP1*(CD/R+QY*SD)                  &
                    -ALP2*Q*FY)/PI2
      U(10,1)=(ALP1*XY*CD                         &
     		+ALP2*XI*FZ+Y/F2*X11)/PI2
      U(11,1)=          (ALP2*EZ)/PI2
...
PURE SUBROUTINE  UB(XI,ET,Q,U,c0,c2)
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8,intent(in) :: XI,ET,Q
      real*8,intent(out) :: U(12,3)
      real*8,intent(in) :: c0(12),c2(24)
!     type(c0_type),intent(in) :: c0
!     type(c2_type),intent(in) :: c2
      !DIMENSION U(12,3)
!C
!C********************************************************************
...
PURE SUBROUTINE  UC(XI,ET,Q,Z,U,c0,c2)
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8,intent(in) :: XI,ET,Q,Z
      real*8,intent(out) :: U(12,3)
      real*8,intent(in) :: c0(12),c2(24)
!     type(c0_type),intent(in) :: c0
!     type(c2_type),intent(in) :: c2
...
PURE SUBROUTINE  DCCON0(ALPHA,DIP,c0)
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8,intent(in) :: ALPHA,DIP
      real*8,intent(inout) :: c0(12)
!     type(c0_type),intent(inout) :: c0
...
PURE SUBROUTINE  DCCON2(XI,ET,Q,SD,CD,c2)
IMPLICIT REAL*8 (A-H,O-Z)
real*8,intent(inout) ::  XI,ET,Q,SD,CD
real*8,intent(out) :: c2(24)
!type(c2_type),intent(inout) :: c2
!C
!C**********************************************************************
!C*****   CALCULATE STATION GEOMETRY CONSTANTS FOR FINITE SOURCE   *****
!C**********************************************************************
