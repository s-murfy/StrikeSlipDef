...
[W2Es_loc,W2Etekr_ux_ss,W2Etekr_uy_ss,W2Etekr_uz_ss,W2E_dux_ss,W2E_duy_ss,W2E_duz_ss]= W2E_geodetic(ss_file ,f_lon_ave);
[E2Ws_loc,E2Ws_dist,E2Wtekr_ux_ss,E2Wtekr_uy_ss,E2Wtekr_uz_ss,E2W_dux_ss,E2W_duy_ss,E2W_duz_ss]= E2W_geodetic(ss_file ,f_lon_ave,f_lat_ave);

% read in displacment due to tensile dislocation 
ten_file = '../res/obs_tekr_ten_2000.dat';
[W2Es_loc,W2Etekr_ux_ten,W2Etekr_uy_ten,W2Etekr_uz_ten,W2E_dux_ten,W2E_duy_ten,W2E_duz_ten]= W2E_geodetic(ten_file,f_lon_ave);
[E2Ws_loc,E2Ws_dist,E2Wtekr_ux_ten,E2Wtekr_uy_ten,E2Wtekr_uz_ten,E2W_dux_ten,E2W_duy_ten,E2W_duz_ten]= E2W_geodetic(ten_file,f_lon_ave,f_lat_ave);
...
function[W2E_sources,W2E_ux,W2E_uy,W2E_uz,dux,duy,duz]= W2E_geodetic(fname,f_lon_ave)
...
tekr_dux = fliplr(dux');  % flip array so it starts from east and goes west
tekr_duy = fliplr(duy');  % flip array so it starts from east and goes west
tekr_duz = fliplr(duz');  % flip array so it starts from east and goes west
dux = tekr_dux;
duy = tekr_duy;
duz = tekr_duz;
...
function[E2W_sources,dist_sources,E2W_ux,E2W_uy,E2W_uz,dux,duy,duz]= E2W_geodetic(fname,f_lon_ave,f_lat_ave)
%  Function calculating the surface displacement at point for a source moving from
...
W2E_sources = fliplr(f_lon_ave);  % flip array so it starts from east and goes west

[s_x,s_y,zone ] = deg2utm(f_lat_ave,f_lon_ave);
for i = 1:length(f_lon_ave)
   dist(i) = sqrt((s_x(i)-s_x(1)).^2+(s_y(i)-s_y(1)).^2);
end
